...
; your answer. (Assume that the evaluation rule for the special form if is the same whether the
...
; (test 0 (p)) => (test 0 p) => (test 0 p) ... infinite loop

; in normal order, (p) is not evaluated untill it needed
; so we are safe from infinite loops
